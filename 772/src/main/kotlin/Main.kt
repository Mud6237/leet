// https://leetcode.com/problemset/all?search=772
			...
			when(t){
				is Token.Operator -> if (t.op == OperatorKind.Plus) sb.append("+") else {
					if (t.op == OperatorKind.UnaryMinus) {
						sb.append("-")	
					} else if (t.op == OperatorKind.Multiply) {
                        sb.append("*")
					} else if (t.op == OperatorKind.Divide) {
                        sb.append("/")
					} else if (t.op == OperatorKind.BinaryMinus) {
                        sb.append(" - ")
                    }
				}
				is Token.Operand -> {
				...
				is Token.Operator -> {
					...
					stack.add(when(t.op) {
						...
						OperatorKind.Multiply -> {
							val l = stack.removeLast() as Token.Operand
							Token.Operand(multiply(l.v, r.v))
						}
						OperatorKind.Divide -> {
							val l = stack.removeLast() as Token.Operand
							Token.Operand(divide(l.v, r.v))
						}
					})
				}
			}
...
fun main(args: Array<String>) {
	...
	println(Solution().calculate("6-4/2"))
	println(Solution().calculate("2*(5+5*2)/3+(6/2+8)"))
}
